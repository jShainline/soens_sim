...
if num_jjs__syn == 2:
    ...
    tau_si_vec = [1e-6] # [1e-6,2e-6,4e-6] # np.linspace(2100e-9,2500e-9,num_tau_si)
    I_sy_vec = [74e-6] # np.arange(70,79,1)*1e-6 # [73e-6,77e-6,81e-6] # np.arange(73e-6,85e-6,1e-6) # [28e-6,30e-6,32e-6,34e-6,36e-6]
    # I_sy_vec = np.arange(70e-6,72e-6,1e-6)
    I_nf = 73e-6
    I_rf = 73e-6
    L_nf = 57.5e-12 # 10e-9 # 
    L_ri = 57.5e-12 # 10e-9 # 
...
for ii in range(num_L_si):
    ...
    for jj in range(num_tau_si):
        ...
        for kk in range(num_I_sy):
            ...
            # if (   (round(I_sy/1e-6) == 74 and round(L_si/1e-9) == 50 and round(tau_si/1e-9) == 1000)  
            #     or (round(I_sy/1e-6) == 74 and round(L_si/1e-9) == 50 and round(tau_si/1e-9) == 4000) 
            #     or (round(I_sy/1e-6) == 76 and round(L_si/1e-9) == 10 and round(tau_si/1e-9) == 2000) 
            #     or (round(I_sy/1e-6) == 78 and round(L_si/1e-9) == 10 and round(tau_si/1e-9) == 2000) 
            #     ):
            #     plot_neuronal_response__single_synaptic_pulse(neuron_1)    
